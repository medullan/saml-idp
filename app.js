...
function _runServer(argv) {
  ...
  const executeSamlEntryIfAuthenticated = (req, res, next) => {
    if (req.user) {
      return samlp.auth(idpOptions)(req, res);
    }
    return next();
  };
  ...
  app.get(
    '/login',
    executeSamlEntryIfAuthenticated,
    (req, res) => {
        ...
        return user.verifyPassword(req.body.password)
          .then(function(valid) {
            if (valid) {
              ...
              req.session.sessionParticipants = [
                {
                  serviceProviderId : 'http://sp-one.com:5000', // The Issuer (Session Participant id)
                  nameId: req.user.nameID, // NameId Of the logged in user in the SP
                  nameIdFormat: 'urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress', // Format of the NameId
                  sessionIndex: req.user.sessionIndex, // The session index generated by the IdP
                  serviceProviderLogoutURL: 'http://sp-one.com:5000/logout', // The logout URL of the Session Participant
                ...
                {
                  serviceProviderId : 'http://uni-nav.com:3000', // The Issuer (Session Participant id)
                  nameId: req.user.nameID, // NameId Of the logged in user in the SP
                  nameIdFormat: 'urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress', // Format of the NameId
                  sessionIndex: req.user.sessionIndex, // The session index generated by the IdP
                  serviceProviderLogoutURL: 'http://uni-nav.com:3000/v1/auth/user/logout?appId=e8b836cd6d20f3431e0fbcb54196360b&clientKey=7c2759273aaf770093f92e0accca965255fac0d1&ref=sp-one.com:5000', // The logout URL of the Session Participant
                  // cert: sp1_credentials.cert // The Session Participant public certificate, used to verify the signature of the SAML requests made by this SP
                }
            ];
              return next();
